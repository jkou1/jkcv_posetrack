...
def make_parser():
    ...
    parser.add_argument("--output_path", default=None, type=str, help="Path to save results")
...
def write_results(filename, results):
    ...
    if os.path.exists(filename) and os.path.getsize(filename) > 0:
        logger.info(f"Results already exist in {filename}, skipping write.")
        return
...
def image_demo(predictor, vis_folder, current_time, args,scene):
    ...
    if args.output_path:
        out_path = osp.join(args.output_path, 'result/detection', 'scene_0' + "{:02d}".format(scene))
    else:
