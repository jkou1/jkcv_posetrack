...
def main():
    ...
    if args.output_path:
        save_root = os.path.join(args.output_path, "result/reidagw")
    else:
        save_root = os.path.join(root_path, "result/reidagw")
    
    scenes = sorted(os.listdir(det_root))
    scenes = [s for s in scenes if s[0]=="s"]
    scenes = scenes[args.start-61:args.end-61]

    reid=torch.load('../ckpt_weight/agw-aic24.pkl',map_location='cpu').cuda().eval()
